...
This folder contains text files for prompt experiments.
...
```bash
python legacy/run_ollama.py --model llama3.2:latest --prompt-file experimentals/manual/long_prompt.txt --max-tokens 200
```
...
The scripts will automatically look in the `experimentals/` folder if the file isn't found in the current directory.
